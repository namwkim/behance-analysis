...
prop_file = csv.reader(open("./data/sampled-graph/colors-100.csv", 'r'))
...
proj_file = csv.reader(open("./data/sampled-graph/designs-100.csv", 'r'))
proj_header = next(proj_file)

output_file = csv.writer(open('./data/sampled-graph/designs-colors-100.csv', 'wb'))
...
for row in proj_file:
	if props_map.has_key(row[0])==False:
		...
		if props[2]=="N/A":
			...
			output_file.writerow(row + props[3:])
			total += 1
