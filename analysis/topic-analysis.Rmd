...
# female vs male
cor.test(as.numeric(factor(names(maleFieldRanks), levels=names(maleFieldRanks))),
         as.numeric(factor(names(maleFieldRanks), levels=names(fieldRanks))), method="kendall")


dat <- data.frame(
    group = rep(c("male", "female"), each=length(maleFieldRanks)),
    x = setNames(c(maleFieldRanksN, femaleFieldRanksN),c(names(maleFieldRanks), names(femaleFieldRanks))),
    y = c(maleFieldRanks, -femaleFieldRanks)
...
cbind(names(maleFieldRanks[1:20]), names(femaleFieldRanks[1:20]))
...
levels(filtered$group)<-c(paste('[',xrange[1,1], ',', xrange[2,1], ']'),
                          paste('[',xrange[1,2], ',', xrange[2,2], ']'),
                          paste('[',xrange[1,3], ',', xrange[2,3], ']'))
...
cohen.d(group2$diversity, group3$diversity)
...
generalist<-group3[group3$diversity>2.0 & group3$project_counts>620,]
nrow(generalist)
...
pairwise.t.test(group3$diversity, group3$gender)
...
levels(filtered$group)<-c(paste('[',round(xrange[1,1],2), ',', round(xrange[2,1],2), ']'),
                          paste('[',round(xrange[1,2],2), ',', round(xrange[2,2],2), ']'),
                          paste('[',round(xrange[1,3],2), ',', round(xrange[2,3],2), ']'))
summary(filtered$homophily)
...
cohen.d(group2$homophily, group3$homophily)
