...
# only 10% percent
users = random.sample(users, int(len(users)*0.1))
for user in users:
    progress+=1
    print '----------1.',progress, '----------'
...
userwriter = csv.writer(open("./data/induced-graph/users-10.csv", 'wb'));
...
desigwriter = csv.writer(open("./data/induced-graph/designs-10.csv", 'wb'));
...
linkwriter = csv.writer(open("./data/induced-graph/links-10.csv", 'wb'));
